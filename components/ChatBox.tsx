...
export function ChatBox() {
  ...
  const groupedMessages =
    currentChat?.chatHistory.reduce((groups: any[], msg, idx) => {
      ...
      const dateKey = timestamp > 0 ? new Date(timestamp).setHours(0, 0, 0, 0) : 0; // Use start of day as key for grouping
            ...
            {canCreateChat() ? (
              ...
              <div className="bg-amber-900/20 border border-amber-500/50 rounded-lg p-4 text-amber-300 text-sm">
                ...
                <p className="text-amber-200/80">
                  Delete existing chats to continue. This is a limitation for the MVP version.
                </p>
              </div>
            )}
          </div>
          ...
          <form onSubmit={handleSubmit} className="flex gap-2 sm:gap-3">
            <div className="flex-1 relative">
              ...
              <motion.button
                ...
                whileHover={{ scale: 1.05 }}
                whileTap={{ scale: 0.95 }}
                className="absolute right-2 sm:right-3 bottom-2 sm:bottom-3 bg-linear-to-r from-purple-600 to-blue-600 hover:from-purple-700 hover:to-blue-700 disabled:opacity-50 disabled:cursor-not-allowed p-2 rounded-lg transition-all shadow-lg flex items-center justify-center z-10"
                title="Send message"
              >
